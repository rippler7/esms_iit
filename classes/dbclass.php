<?php
	...
	if ( ! function_exists ('pg_connect') ) {
	   die('<b>Fatal Error:</b> Postgres DB requires PostgreSQL Library to be compiled and or linked in to the PHP engine');
    }

	class pgsql_db 
	{
		var $db_name = "msuiit"; # default db name
		var $db_user = "esmsweb";   # default db user
		var $db_pwd  = 'w3b3$m$';    # default db password
		var $db_host = "203.177.109.40"; # default db host 
		...
		# Description: DB Constructor - connects to the server and selects a database
		{
			...
			if ( !$this->connect() )
			{
				die ("<ol><strong>Error establishing a database connection!</strong>
		...
		function connect($dbuser='', $dbpwd='', $dbname='', $dbhost='')
		{
			...
			{       
                ...
                $this->db_conn = @pg_connect("host=".$this->db_host." user=".$this->db_user.
				                            " password=".$this->db_pwd." dbname=".$this->db_name); 
